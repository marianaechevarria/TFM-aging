import os
os.environ['OPENBLAS_NUM_THREADS'] = '1'                # for use with multiprocessing
from brian2 import *
import sys
import numpy as np
import time
...
def run_sims(i_sims):  
    ...
    K=500    # total number of synaptic inputs             #as in H&M
    ...
    KE=int(ceil(0.8*K*0.7))   # number of excitatory inputs
...
def main():
    ...
    numcores = int(sys.argv[1])         #?| get the number of cores
    #numcores = mp.cpu_count()
    num_trials = int(sys.argv[2])       #?| get the number of trials
    print('numcores: ', numcores)

    pool = mp.Pool(processes=numcores, maxtasksperchild=1)
    pool.map(run_sims, range(num_trials), chunksize=1)
